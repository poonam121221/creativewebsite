	...
	function getSingleList($table, $filter=array(),$order=array()){
		...
		if(trim($table) !="" && trim($table) !=null && count($filter)>0){
			...
			if(checkLanguage("english")){
				$column = ",page_title_en as title, page_description_en as description";
			}else{
				$column = ",page_title_hi as title, page_description_hi as description";
			}
	...
	function ajax_search_by_title($params = array(),$filter=array(),$orderBy=array()){
		...
		if(array_key_exists("table",$params)){
	        ...
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	            $this->db->like('page_title_hi',trim($params['search']['title']));
	        }
	        
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	            $this->db->or_like('page_title_en',trim($params['search']['title']));
	        }
	        
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	            $this->db->or_like('page_description_hi',trim($params['search']['title']));
	        }
	        
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	            $this->db->or_like('page_description_en',trim($params['search']['title']));
	        }
