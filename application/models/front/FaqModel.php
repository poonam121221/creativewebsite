...
class FaqModel extends MY_Model {
	...
	function getSingleList($table, $filter=array(),$order=array()){
		...
		if(trim($table) !="" && trim($table) !=null && count($filter)>0){
			...
			if(checkLanguage("english")){
				$column = ",title_en as question, answer_en as answer";
			}else{
				$column = ",title_hi as question, answer_hi as answer";
			}
	...
	function ajax_search_by_title($params = array(),$filter=array(),$orderBy=array()){
		...
		if(array_key_exists("table",$params)){
			...
			$column = "";
			if(checkLanguage("english")){
				$column = "title_en as title,cat_title_en as cat_title";
			}else{
				$column = "title_hi as title,cat_title_hi as cat_title";
			}	
			
	        $this->db->select('faq.*,'.$column);
	        $this->db->from($params['table'].' faq');
	       $this->db->join('comm_faq_category ', 'comm_faq_category.cat_id = faq.cat_id','left');
