...
class ProjectMediaModel extends MY_Model {
	...
	function getAllList($table, $filter=array()){
		...
		if(trim($table) !="" && trim($table) !=null){
			$this->db->select('um.*,cp.title_en as project_name,concat(cm.admin_fname," ",cm.admin_lname) as admin_name, concat(cm1.admin_fname," ",cm1.admin_lname) as edited_name');

			$this->db->from($table.' um');
			 $this->db->join('comm_project cp', 'cp.id = um.project_id','left');
