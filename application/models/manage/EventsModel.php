...
class EventsModel extends MY_Model {
	...
	function ajax_search_by_title($params = array(),$filter=array(),$orderBy=array()){
		...
		if(array_key_exists("table",$params)){
			...
			$this->db->select('ev.*, concat(cm.admin_fname," ",cm.admin_lname) as admin_name, concat(cm1.admin_fname," ",cm1.admin_lname) as edited_name,cpgc.cat_title_en, cpgc.cat_title_hi');
			$this->db->from($params['table'].' ev');
			$this->db->join('comm_admin cm', 'cm.admin_id = ev.added_by','left');
			$this->db->join('comm_admin cm1', 'cm1.admin_id = ev.edit_by','left');
	   	    $this->db->join('comm_project_category cpgc', 'cpgc.cat_id = ev.cat_id','left');
	       
	        //filter data by searched keywords
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	            $this->db->like('ev.title_hi',trim($params['search']['title']));
	        }
	        
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	            $this->db->or_like('ev.title_en',trim($params['search']['title']));
	        }
	        
	        if(isset($params['search']['status']) && trim($params['search']['status'])!=""){
	        $this->db->where('ev.status', $params['search']['status']);
	        }
