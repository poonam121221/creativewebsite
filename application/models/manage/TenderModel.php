...
class TenderModel extends MY_Model {
	...
	function getRows($params = array(),$filter=array(),$orderBy=array()){
		if(array_key_exists("table",$params)){
			...
			if(checkLanguage("english")){
			 $col_name = ',um.title_en as title,um.remark_en as remark';
			}else{
			 $col_name = ',um.title_hi as title,um.remark_hi as remark';
			}
			
	        $this->db->select('um.*, concat(cm.admin_fname," ",cm.admin_lname) as admin_name, concat(cm1.admin_fname," ",cm1.admin_lname) as edited_name'.$col_name);
	        ...
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	            $$search_where .= '(um.title_hi LIKE "%'.trim($params['search']['title']).'%" ';
	        }
	        
	        if(isset($params['search']['title']) && trim($params['search']['title'])!=""){
	           $search_where .= 'or um.title_en LIKE "%'.trim($params['search']['title']).'%" )';
