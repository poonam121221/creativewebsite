...
class NewsModel extends MY_Model {
	...
	function getAllList($table, $filter=array(),$orderBy=array()){
		...
		if(trim($table) !="" && trim($table) !=null){
			$this->db->select('n.*, concat(cm.admin_fname," ",cm.admin_lname) as admin_name, concat(cm1.admin_fname," ",cm1.admin_lname) as edited_name');
			$this->db->from($table.' n');
			$this->db->join('comm_admin cm', 'cm.admin_id = n.added_by','left');
			$this->db->join('comm_admin cm1', 'cm1.admin_id = n.edit_by','left');
			if(count($filter)>0){
				$this->db->where($filter);
			}
			if(count($orderBy)>0){
