...
class TestimonialModel extends MY_Model {
	...
	function getAllList($table, $filter=array(),$orderBy=array()){
		...
		if(trim($table) !="" && trim($table) !=null){
			$this->db->select('cp.*, concat(cm.admin_fname," ",cm.admin_lname) as admin_name, concat(cm1.admin_fname," ",cm1.admin_lname) as edited_name');
	...
	function ajax_search_by_title($params = array(),$filter=array(),$orderBy=array(),$adminRec=FALSE){
		...
		if(array_key_exists("table",$params)){
	        ...
	        $this->db->select('cp.*'.$column.$adminColumn);
