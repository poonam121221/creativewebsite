...
class ServicesModel extends MY_Model {
	...
	function getAllCatgeoryList($table, $filter=array(),$orderBy=array()){
		
		if(trim($table) !="" && trim($table) !=null){
			$this->db->select('s.*, concat(cm.admin_fname," ",cm.admin_lname) as admin_name, concat(cm1.admin_fname," ",cm1.admin_lname) as edited_name , cs.cat_title_en as catgeory_name');
			$this->db->from($table.' s');

			$this->db->join('comm_services_category cs', 'cs.cat_id = s.parent_cat_id','left');
			$this->db->join('comm_admin cm', 'cm.admin_id = s.added_by','left');
			$this->db->join('comm_admin cm1', 'cm1.admin_id = s.edit_by','left');
			if(count($filter)>0){
				$this->db->where($filter);
	...
	}// end getAllCatgeoryList function
