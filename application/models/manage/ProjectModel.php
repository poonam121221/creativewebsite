...
class ProjectModel extends MY_Model {
	...
	function getSingleList1($table, $filter=array()){
	...
	function getSingleList($table, $filter=array(),$order=array()){
		...
		if(trim($table) !="" && trim($table) !=null && count($filter)>0){
			...
			if(checkLanguage("english")){
				$column = ",title_en as title, description_en as description,cpgc.cat_title_en as cat_title";
			}else{
				$column = ",title_hi as title, description_hi as description,cpgc.cat_title_hi,cpgc.cat_title_hi as cat_title";
			}
			
			$this->db->select('cp.*'.$column);
			$this->db->from($table.' cp');
		  $this->db->join('comm_project_category cpgc', 'cpgc.cat_id = cp.cat_id','left');
			$this->db->where($filter);
