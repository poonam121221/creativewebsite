        ...
        <div class="row">
            ...
            <div class="col-md-9 no-padding" id="ele1">
                <div class="aboutus-midinner-wrapper">
                    <h2><?php echo $this->lang->line('emergency_contact'); ?></h2>
                    <div class="row">
                        <div class="col-sm-6">
                           
                             <?php 
                                    echo form_dropdown('district',
                                isset($district)?$district:array(''=>$this->lang->line('all_district')),isset($searchdistrict) && $searchdistrict!=''?$searchdistrict:'',array('class'=>'form-control','id'=>'selectDistrict'));
...
//Do not use this function in Dom
    function searchFilter(page_num, all) {
        ...
        var dataString = {'page': page_num, 'sTitle': sTitle,'selectDistrict':selectDistrict,'<?php echo $this->security->get_csrf_token_name(); ?>':tokenVal };

        $('#ajaxloading').modal('show');

        $.ajax({
            type: 'POST',
            url: '<?php echo base_url("EmergencyContact/ajaxPaginationData/"); ?>' + page_num,
            data: dataString,
