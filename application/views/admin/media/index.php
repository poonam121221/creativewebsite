     ...
     <table id="example" class="table table-striped table-bordered dataTable">
		...
		<?php
		 ...
		 if(in_array($ext,$imgChkArray)){
		 	echo '<a class="fancybox" rel="mygrp" href="'.base_url("uploads/media/").html_escape($rows['attachment']).'" data-rel="fancybox-button">';
		 	echo img(array('src'=>base_url('uploads/media/').html_escape($rows['attachment']),'title'=>'','class' => 'img-thumbnail','width'=>'80','height'=>'50'));
		 	echo '</a>';
		 }else{
		 	echo '<a target="_blank" href="'.base_url("uploads/media/").html_escape($rows['attachment']).'" data-rel="fancybox-button">';
		 	echo img(array('src'=>base_url('webroot/img/download_file.jpg'),'title'=>'','class' => 'img-thumbnail','width'=>'80','height'=>'50'));
		 	echo '</a>';
		 }
		   ?></td>
		 <td><?php echo base_url('uploads/media/').html_escape($rows['attachment']);  ?></td>
	    ...
	    <a href="<?php echo base_url('manage/Media/delete/'.$id.'/'); ?>" class="btn btn-danger btn-condensed" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete Data" onclick="return confirm('Are you sure to delete record?'); ">
