...
<div class="col-md-12">
	<ul class="page-breadcrumb breadcrumb">
		...
		<li><a href="<?php echo base_url('manage/Project/'); ?>">Project</a><i class="fa fa-angle-right"></i></li>
		...
		<li><a href="<?php echo base_url('manage/Project/add'); ?>"><i class="fa fa-plus"></i> Add</a></li>
		</ul>
		</li>
	</ul>
...
<div class="portlet-body">
	...
	<?php
		...
		$atr2 =array('id'=>'frmProject','name'=>'frmProject','role'=>'form', 'autocomplete'=>'off');
   		echo form_open_multipart('manage/Project/add',$atr2,$hidden); 
	?>
     ...
     <div class="form-group clearfix">
		<label class="col-lg-2 col-md-3 control-label">Start Date <span class="red">*</span></label>
		<div class="col-lg-5 col-md-6">
		<?php $START_DATE = array('name'=>'s_date','id'=>'s_date','class'=>'form-control');
	...
	<div class="form-group clearfix">
		<label class="col-lg-2 col-md-3 control-label">End Date <span class="red">*</span></label>
		<div class="col-lg-5 col-md-6">
		<?php $END_DATE = array('name'=>'e_date','id'=>'e_date','class'=>'form-control');
	...
	<div class="form-group clearfix">
     ...
     <div class="form-group clearfix">
     <label class="col-lg-2 col-md-3 control-label"> Project Status <span class="red">*</span></label>  
     <div class="col-lg-5 col-md-6">
     <?php $PSTATUS = array(""=>"--Select Status--",'1'=>'Completed','2'=>'In Progress');
		echo form_dropdown('project_status', $PSTATUS, (isset($DataList->project_status) && $DataList->project_status !='' )?
         html_escape($DataList->project_status):set_value('status'),array('class'=> 'form-control'));
	    ?>
     </div>
     </div>
     <?php if($optstatus==1){ ?>
     <div class="form-group clearfix">
     <label class="col-lg-2 col-md-3 control-label"> Publish Status <span class="red">*</span></label>  
     <div class="col-lg-5 col-md-6">
     <?php $STATUS = array(""=>"--Select Status--",'1'=>'Publish','0'=>'Pending');
		echo form_dropdown('status', $STATUS, (isset($DataList->status) && $DataList->status !='' )?
         html_escape($DataList->status):set_value('status'),array('class'=> 'form-control'));
	    ?>
     </div>
     </div><!--End Form-group-->
	 ...
	 <a class="btn purple" href="<?php echo base_url('manage/Project/'); ?>"> Back</a>
     </div>
     </div><!--End Form-group-->
...
$(document).ready(function(){
	...
	$('#s_date').datetimepicker({autoclose:true,format:"dd-mm-yyyy hh:ii",startDate:'0d',minuteStep:5,pickerPosition:"top-left"})
	.on('changeDate', function(ev) {
	    if($('#s_date').valid()){
	       $('#s_date').removeClass('help-block');   
	    }
    });
	$('#e_date').datetimepicker({autoclose:true,format:"dd-mm-yyyy hh:ii",startDate:'0d',minuteStep:5,pickerPosition:"top-left"})
	.on('changeDate', function(ev) {
	    if($('#e_date').valid()){
    ...
    jQuery.validator.addMethod("afterBeginDateTime",function(value, element, param){
    	var status = false;
    	if(( $('#'+param[1]).data('datetimepicker').getDate() - $('#'+param[0]).data('datetimepicker').getDate())<=1){
			status = false;
		}else{
			status = true;
		}		
