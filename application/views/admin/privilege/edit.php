<?php
   if(isset($PrivilegeData)):
     // fatch Privilege data from database
     $row = $PrivilegeData;
	 $UmpMList = explode(",",$row->upm_range);         
   endif;
	...
	function buildTree($parent, $menu,$UmpMList) {
	$html ="";
	if (isset($menu['parent_menus'][$parent])) {
			$html .= "<ul>";
		foreach ($menu['parent_menus'][$parent] as $menu_id) {
			if (!isset($menu['parent_menus'][$menu_id])) {
				$MyVal = (string)$menu['menus'][$menu_id]['id'];
								
				$html .= "<li>";
				$html .= "<input ";
				if (in_array($MyVal, $UmpMList)) {
			...
			if (isset($menu['parent_menus'][$menu_id])) {
				$MyVal2 = (string)$menu['menus'][$menu_id]['id'];
				$html .= "<li>";
				$html .= "<input ";
				if (in_array($MyVal2, $UmpMList)) {
					 $html .= "checked='checked'";
				}				
				$html .= "type='checkbox' value=";
				$html .= $menu['menus'][$menu_id]['id'];
				$html .= " name='ids[]'>";
				$html .= "<span>".$menu['menus'][$menu_id]['menu_name']."</span>";
				$html .= buildTree($menu_id, $menu,$UmpMList);
				$html .= "</li>";
			}
		}
		$html .= "</ul>";
...
<div class="portlet-title">
  <div class="caption">Edit User Privilege</div>
...
<?php
$hidden = array('id' => html_escape(isset($PrivilegeData->upm_id)? encrypt_decrypt('encrypt',$PrivilegeData->upm_id):''));
$atr2 =array('id'=>'frmPrivilege','name'=>'frmPrivilege','class'=>'form-horizontal','role'=>'form', 'autocomplete'=>'off');
echo form_open('manage/Userprivilege/edit',$atr2,$hidden); 
?>
 <div class="form-body">
 	...
 	<div class="form-group">
		...
		<?php $UPM_NAME = array( 
        'name'=>'uname','id'=>'uname','class'=> 'form-control input-medium','placeholder'=>'Enter Sate Name',
        'value' => (isset($PrivilegeData->upm_name) && $PrivilegeData->upm_name !='' )?
         html_escape($PrivilegeData->upm_name):set_value('upm_name'));
	...
	<div class="form-group clearfix">
     ...
     <?php $UPM_DESCRIPCTION = array(
        ...
        'rows'          => 4,
        'cols'          => 10,
        'value'         => (isset($PrivilegeData->upm_description) && $PrivilegeData->upm_description !='' )?
         html_escape($PrivilegeData->upm_description):set_value('upm_description'));
 	...
 	<div class="form-group">
		<label class="col-sm-4 col-md-3 control-label">Is delete</label>
		<div class="col-sm-8 col-md-7">
		<?php $OPTIONS = array('1'=>'Yes','0'=>'No');
		echo form_dropdown('status', $OPTIONS, (isset($PrivilegeData->isdelete) && $PrivilegeData->isdelete !='' )?
         html_escape($PrivilegeData->isdelete):set_value('isdelete'),array('class'=> 'form-control input-medium'));
	    ?>
		</div>
	</div><!--End form-group-->
      ...
      <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div id="tree">
						   <?php echo buildTree(0, $menu,$UmpMList); ?>
						</div><!--End Tree-->
                    </div>
