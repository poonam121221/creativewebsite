...
class Notification extends Company_Controller{
	...
	private $__userPanelType = 2;//2=user panel
	...
	public function __construct(){
		...
		$this->perPage = 10;
		$this->__loggedUserId = encrypt_decrypt('decrypt',$this->session->userdata['AUTH_LOCAL_USER']['SERIALNO']);
	}//end constructor
	
	public function index(){ 
	  //Create dynamic Breadcrumbs
	 $this->breadcrumbs->push($this->lang->line('notification'), '/');
	 $this->breadcrumbs->unshift($this->lang->line('dashboard'), '/');
	 
	 $UserName = $this->session->userdata['AUTH_LOCAL_USER']['USER_NAME'];
	 ...
	 $this->__queryStatus = $this->NotificationModel->updatedata($this->__table,$DATAINPUT,$filter,$logActivity,TRUE,$this->__loggedUserId,'user');
	 
     $this->front_view('user/notification/index',$this->data);
	}//end index function
	...
	public function ajaxNotification(){
        ...
        $config['base_url']    = base_url("user/Notification/ajaxNotification");
       ...
       $this->data['PageNo'] = $offset;
        ...
        //load the view
        $this->load->view('user/notification/ajax_notification', $this->data, false);
