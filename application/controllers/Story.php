...
class Story extends Frontend_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('front/StoryModel');
	...
	public function index(){
		...
		$this->breadcrumbs->push($this->lang->line('success_story'), '/');
		$this->breadcrumbs->unshift($this->lang->line('home_page'), '/');
		
		$filter = array('status'=>1,'is_delete'=>0);
		$orderBy = array('order_preference'=>'asc');
		$this->data['DataList'] = $this->StoryModel->getSingleList($this->__table,$filter,$orderBy);
		
		$this->front_view('public/story/index',$this->data);
	...
	public function view(){
		...
		$this->data['DataList'] = $this->StoryModel->getSingleList($this->__table,$filter);
		
		$this->front_view('public/story/view',$this->data);
	}//end about function
	...
	public function ajaxPaginationData(){
        ...
        //total rows count
        $totalRec = count($this->StoryModel->ajax_search_by_title($conditions,$filter,$orderBy));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url("Story/ajaxPaginationData");
        ...
        //get posts data
        $this->data['DataList'] = $this->StoryModel->ajax_search_by_title($conditions,$filter,$orderBy);
        $this->data['PageNo'] = $offset;
        
        //load the view
        $this->load->view('public/story/ajax_story', $this->data, false);
