...
class Rulesactscategory extends Admin_Controller{
	
	private $__queryStatus = FALSE;
	private $__table = "comm_rules_acts_category";
	private $__id = NULL;
	...
	public function index(){
		...
		$filter = array('ra.is_delete'=>0);
		//this is not for super admin and administrator
		if(!in_array($this->__LogedPrivId,$this->__allowChkStatus)){
			$filter['ra.added_by'] = $UserLogId ; 
		}
		
		$this->data['DataList'] = $this->RulesactsModel->getAllList($this->__table,$filter);		
	  	$this->front_view('admin/rules_acts_category/index',$this->data);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Rulesactscategory/add/');
         }else{
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         	    $DATAINPUT = array(
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/rules_acts_category/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			...
			redirect('manage/Rulesactscategory/');
		}
		...
		$this->data['DataList'] = $this->RulesactsModel->getSingleList($this->__table,array('cat_id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Rulesactscategory/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->RulesactsModel->updatedata($this->__table,$DATAINPUT,array('cat_id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/rules_acts_category/edit',$this->data);
	}//end edit function
	...
	public function delete(){
		...
		if($this->RulesactsModel->deletedata($this->__table,array('cat_id'=>$this->__id))==TRUE){
	...
	public function delete(){
		...
		$count_record = $this->CircularModel->record_count($this->__tblGallery,$filter);
		if($count_record==0){
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Sorry!'));
			redirect('manage/Rulesactscategory/recycle');
		}
