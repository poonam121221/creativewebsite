...
class PublicationCategory extends Admin_Controller{
	
	private $__queryStatus = FALSE;
	private $__table = "comm_publications_category";
	private $__tblpublications = "comm_publications";
	private $__id = NULL;
	...
	public function index(){
		...
		$this->data['DataList'] = $this->PublicationModel->getAllCategoryList($this->__table,$filter);		
	  	$this->front_view('admin/publications_category/index',$this->data);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/publicationscategory/add/');
         }else{
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         	    $DATAINPUT = array(
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/publications_category/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			...
			redirect('manage/publicationscategory/');
		}
		...
		$this->data['DataList'] = $this->PublicationModel->getSingleList($this->__table,array('cat_id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/publicationscategory/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->PublicationModel->updatedata($this->__table,$DATAINPUT,array('cat_id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/publications_category/edit',$this->data);
	}//end edit function
	...
	public function delete(){
		...
		$count_record = $this->PublicationModel->record_count($this->__tblpublications,$filter);
		if($count_record==0){
	...
	public function recycle(){
		...
		$this->data['DataList'] = $this->PublicationModel->getAllCategoryList($this->__table,$filter);		
	  	$this->front_view('admin/publications_category/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Something went wrong, try again!'));
			redirect('manage/publicationscategory/recycle');
		}
		...
		$filter = array('cat_id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	...
		 	$count_record = $this->PublicationModel->record_count($this->__tblpublications,$filter);
		 	if($count_record==0){
				if($this->PublicationModel->deletedata($this->__table,array('cat_id'=>$this->__id))==TRUE){
					$this->session->set_flashdata('AppMessage',array('class'=>'success',
