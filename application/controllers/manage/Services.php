...
class Services extends Admin_Controller{
	...
	private $__table = "comm_services";	
	private $__catTable = "comm_services_category";
	private $__id = NULL;
	...
	public function index(){          
        //load the view
        $this->front_view('admin/services/index',$this->data);	  	
	}//end index function
	...
	public function ajaxPaginationData(){
        ...
        //total rows count
        $totalRec = count($this->ServicesModel->getRows($conditions,$filter));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url()."manage/Services/ajaxPaginationData";
        ...
        //get posts data
        $this->data['DataList'] = $this->ServicesModel->getRows($conditions,$filter,$orderBy);
        $this->data['PageNo'] = $offset;
        
        //load the view
        $this->load->view('admin/services/ajaxpaginationservices', $this->data, false);
	...
	public function add(){
		...
		$this->data['CategoryList'] = $this->ServicesModel->GenerateDDList($this->__catTable,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1,'cat_id not IN (1,2,3,4,5,6)  AND is_delete ='=>0));	
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Services/add/');
         }else{
				
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         		$result =  $this->ServicesModel->getSingleList($this->__table);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/services/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['CategoryList'] = $this->ServicesModel->GenerateDDList($this->__catTable,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1,'cat_id not IN (1,2,3,4,5,6)  AND 1 ='=>1));
		...
		$this->data['DataList'] = $this->ServicesModel->getSingleList($this->__table,array('id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Services/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->ServicesModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/services/edit',$this->data);
	}//end edit function
	...
	public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('order_number', 'Order Number', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->ServicesModel->update_sort_order($update_id,$update_order,$this->__table);
	...
	public function recycle(){  
        ...
        $this->data['DataList'] = $this->ServicesModel->getAllList($this->__table,$filter,$order);        
        //load the view
        $this->front_view('admin/services/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Sorry!'));
			 redirect('manage/Services/recycle');
		}
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
			...
			$Query = $this->ServicesModel->getSingleList($this->__table,$filter);
			
			if($this->ServicesModel->deletedata($this->__table,$filter)==TRUE){
