...
class Circular extends Admin_Controller{
	
	private $__queryStatus = FALSE;
	private $__table = "comm_circulars";
	private $__cattTbl = "comm_circular_category";
	private $__id = NULL;
	...
	public function index(){    
        ...
        $this->data['CategoryList'] = $this->CircularModel->GenerateDDList($this->__cattTbl,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1));
        $this->front_view('admin/circular/index',$this->data);
	}//end index function
	...
	public function ajaxPaginationData(){
        ...
        //total rows count 
        $totalRec = $this->CircularModel->get_filtered_data($conditions,$filter);
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url()."manage/Circular/ajaxPaginationData";
        ...
        //get posts data
        $this->data['DataList'] = $this->CircularModel->make_datatable($conditions,$filter,$orderBy);
        $this->data['PageNo'] = $offset;
        
        //load the view
        $this->load->view('admin/circular/ajaxpaginationcircular', $this->data, false);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Circular/add/');
         }else{
				
				$s_order = (int)$this->CircularModel->getmax($this->__table,'order_preference');
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/circular/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['DataList'] = $this->CircularModel->getSingleList($this->__table,array('id'=>$this->__id));
		$this->data['CategoryList'] = $this->CircularModel->GenerateDDList($this->__cattTbl,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Circular/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->CircularModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/circular/edit',$this->data);
	}//end edit function
	...
	public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('order_number', 'Order Number', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->CircularModel->update_sort_order($update_id,$update_order,$this->__table);
	...
	public function recycle(){    
        //load the view
        $this->front_view('admin/circular/recycle',$this->data);
	}//end index function
	...
	public function ajaxPaginationRecycle(){
        ...
        //total rows count
        $totalRec = count($this->CircularModel->getRows($conditions,$filter));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url()."manage/Circular/ajaxPaginationRecycle";
        $config['total_rows']  = $totalRec;
        $config['uri_segment'] = 4;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
        ...
        //get posts data
        $this->data['DataList'] = $this->CircularModel->getRows($conditions,$filter,$orderBy);
        $this->data['PageNo'] = $offset;
        
        //load the view
        $this->load->view('admin/circular/ajaxpaginationCircularRecycle', $this->data, false);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Sorry!'));
			 redirect('manage/Circular/recycle');
		}
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
			...
			$Query = $this->CircularModel->getSingleList($this->__table,$filter);
			
			if($this->CircularModel->deletedata($this->__table,$filter)==TRUE){
