...
class Publication extends Admin_Controller
{
	...
	private $__catTbl = "comm_publications_category";
	private $__id = NULL;
	...
	{
		parent::__construct();
		$this->load->model('manage/PublicationModel');
	...
	public function index()
	{
		...
		//get the posts data        
		$filter = array('wn.is_delete' => 0);
		if (!in_array($this->__LogedPrivId, $this->__allowChkStatus)) {
			$filter['wn.added_by'] = $UserLogId;
		}

		$order = array('wn.order_preference' => 'asc');
		$this->data['DataList'] = $this->PublicationModel->getAllList($this->__table, $filter, $order);
		$this->data['CategoryList'] = $this->PublicationModel->GenerateDDList($this->__catTbl, 'cat_id', 'cat_title_en', '--SELECT CATEGORY--', array('cat_status' => 1, 'is_delete' => 0));
		//load the view
		$this->front_view('admin/publication/index', $this->data);
	} //end index function
	...
	public function add()
	{
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				redirect('manage/Publication/add/');
			} else {
				$UserLogId =  encrypt_decrypt("decrypt", $this->session->userdata['AUTH_USER']['SERIALNO']);
				$result =  $this->PublicationModel->getSingleList($this->__table);
				...
				$this->__queryStatus = $this->PublicationModel->insertdata($this->__table, $DATAINPUT);
		...
		$this->front_view('admin/publication/add', $this->data);
	} //end add function
	...
	public function edit()
	{
		...
		$this->data['DataList'] = $this->PublicationModel->getSingleList($this->__table, array('id' => $this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->form_validation->run() == FALSE) {
				$this->session->set_flashdata('AppMessage', array('class' => 'warning', 'message' => validation_errors()));
				redirect('manage/Publication/edit/' . $this->__encId . '/');
			} else {
				$UserLogId  =  encrypt_decrypt("decrypt", $this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->PublicationModel->updatedata($this->__table, $DATAINPUT, array('id' => $this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
		$this->front_view('admin/publication/edit', $this->data);
	} //end edit function
	...
	public function updatesrtorder()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->__queryStatus = $this->PublicationModel->update_sort_order($update_id, $update_order, $this->__table);
	...
	public function recycle()
	{
		...
		$this->data['DataList'] = $this->PublicationModel->getAllList($this->__table, $filter, $order);
		//load the view
		$this->front_view('admin/publication/recycle', $this->data);
	...
	public function recycle_delete()
	{
		...
		if (($this->__id == NULL || $this->__id == FALSE ||  $this->__id == "")) {
			$this->session->set_flashdata('AppMessage', array('class' => 'danger', 'message' => 'Sorry!'));
			redirect('manage/Publication/recycle');
		}
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($del_no == 0) {
				if ($this->PublicationModel->updatedata($this->__table, $data, $filter) == TRUE) {
				...
				$Query = $this->PublicationModel->getSingleList($this->__table, $filter);

				if ($this->PublicationModel->deletedata($this->__table, $filter) == TRUE) {
	...
	public function archieve_publication()
	{
	}
