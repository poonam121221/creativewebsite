...
class Videocategory extends Admin_Controller{
	
	private $__queryStatus = FALSE;
	private $__table = "comm_video_gallery_category";
	private $__tblGallery = "comm_video_gallery";
	private $__id = NULL;
	...
	public function index(){
		...
		$this->data['DataList'] = $this->VideogalleryModel->getAllCategoryList($this->__table,$filter);		
	  	$this->front_view('admin/video_category/index',$this->data);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Videocategory/add/');
         }else{
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         	    $DATAINPUT = array(
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/video_category/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['DataList'] = $this->VideogalleryModel->getSingleList($this->__table,array('cat_id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Videocategory/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->VideogalleryModel->updatedata($this->__table,$DATAINPUT,array('cat_id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/video_category/edit',$this->data);
	}//end edit function
	...
	public function delete(){
		...
		$count_record = $this->VideogalleryModel->record_count($this->__tblGallery,$filter);
		if($count_record==0){
	...
	public function recycle(){
		...
		$this->data['DataList'] = $this->VideogalleryModel->getAllCategoryList($this->__table,$filter);		
	  	$this->front_view('admin/video_category/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Something went wrong, try again!'));
			redirect('manage/Videocategory/recycle');
		}
		...
		$filter = array('cat_id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	...
		 	$count_record = $this->VideogalleryModel->record_count($this->__tblGallery,$filter);
		 	if($count_record==0){
				if($this->VideogalleryModel->deletedata($this->__table,array('cat_id'=>$this->__id))==TRUE){
					$this->session->set_flashdata('AppMessage',array('class'=>'success',
