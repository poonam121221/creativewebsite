...
class Contactcategory extends Admin_Controller{
	
	private $__queryStatus = FALSE;
	private $__table = "comm_contact_category";
	private $__id = NULL;
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('manage/ContactcategoryModel');
	...
	public function index(){
		...
		$this->data['DataList'] = $this->ContactcategoryModel->getAllList($this->__table,$filter);		
	  	$this->front_view('admin/contact_category/index',$this->data);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 $this->form_validation->set_rules('category_hi', 'Category Title (Hindi)', 'trim|required|min_length[2]|max_length[100]|is_unique['.$this->__table.'.category_hi]');	
		 $this->form_validation->set_rules('category_en', 'Category Title (English)', 'trim|required|min_length[2]|max_length[100]|is_unique['.$this->__table.'.category_en]');
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Contactcategory/add/');
         }else{
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         	    $DATAINPUT = array(
         	    	'category_hi' => cleanQuery(trim($this->input->post('category_hi',TRUE))),
         	    	'category_en' => cleanQuery(trim($this->input->post('category_en',TRUE))),         	    	
         	    	'added_date'   => date('Y-m-d h:i:s'),
         	    	'added_by'     => $UserLogId,
         	    	'cat_status'   => $this->__status
         	    	);
				
				$this->__queryStatus = $this->ContactcategoryModel->insertdata($this->__table,$DATAINPUT);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/contact_category/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			...
			redirect('manage/Contactcategory/');
		}
		...
		$this->data['DataList'] = $this->ContactcategoryModel->getSingleList($this->__table,array('cat_id'=>$this->__id));
		
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
			
			 $this->form_validation->set_rules('category_hi', 'Category Title (Hindi)', 'trim|required|min_length[2]|max_length[100]|check_unique['.$this->__table.'.category_hi.cat_id.'.$this->__id.']');
			 $this->form_validation->set_rules('category_en', 'Category Title (English)', 'trim|required|min_length[2]|max_length[100]|check_unique['.$this->__table.'.category_en.cat_id.'.$this->__id.']');
			 if($this->__allowStatus==1){
		 		$this->form_validation->set_rules('status', 'Status', 'trim|required|in_list[0,1]');	
		 		$this->__status = (int)cleanQuery($this->input->post('status'));	 
		 	 }else{
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Contactcategory/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->ContactcategoryModel->updatedata($this->__table,$DATAINPUT,array('cat_id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/contact_category/edit',$this->data);
	}//end edit function
	...
	public function delete(){
		...
		if($this->ContactcategoryModel->countCategoryRecord($this->__id)==TRUE){
		   $DATAINPUT = array('is_delete'=>1,'edit_by'=>$UserLogId,'edit_date'=> date('Y-m-d h:i:s'));
		
		   if($this->ContactcategoryModel->updatedata($this->__table,$DATAINPUT,array('cat_id'=>$this->__id))==TRUE){
	...
	public function recycle(){
		...
		$this->data['DataList'] = $this->ContactcategoryModel->getAllList($this->__table,$filter);		
	  	$this->front_view('admin/contact_category/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Sorry!'));
			redirect('manage/Contactcategory/recycle');
		}
		...
		$filter = array('cat_id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	if($this->ContactcategoryModel->updatedata($this->__table,$data,$filter)==TRUE){
		 	...
		 	if($this->ContactcategoryModel->countCategoryRecord($this->__id)==TRUE){
				if($this->ContactcategoryModel->deletedata($this->__table,array('cat_id'=>$this->__id))==TRUE){
					$this->session->set_flashdata('AppMessage',array('class'=>'success',
