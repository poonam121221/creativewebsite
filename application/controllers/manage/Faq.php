...
class Faq extends Admin_Controller{
	...
	public function index(){
		$this->data['CategoryList'] = $this->FaqModel->GenerateDDList($this->__catTbl,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1,'is_delete'=>0));
	  	$this->front_view('admin/faq/index',$this->data);
	}//end index function
	...
	public function ajaxPaginationData(){
        ...
        //total rows count
        $totalRec = count($this->FaqModel->ajax_search_by_title($conditions,$filter,$orderBy,TRUE));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url("Gallerymaster/ajaxPaginationData");
        ...
        //get posts data
        $this->data['DataList'] = $this->FaqModel->ajax_search_by_title($conditions,$filter,$orderBy,TRUE);
        $this->data['PageNo']   = $offset;
        
        //load the view
        $this->load->view('admin/faq/ajax_paginationgallery', $this->data, false);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 $this->form_validation->set_rules('title_en', 'Title (English)', 'trim|required|min_length[2]|max_length[255]');
                 $this->form_validation->set_rules('answer_hi', 'Title (Hindi)', 'trim|required|min_length[2]');	
		 $this->form_validation->set_rules('answer_en', 'Title (English)', 'trim|required|min_length[2]');
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Faq/add/');
         }else{
         			$s_order = (int)$this->FaqModel->getmax($this->__table,'order_preference');
					$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
					...
					$this->__currentUrl = current_url();
					    ...
					    $DATAINPUT = array(
		         	    ...
		         	    'answer_hi'         => cleanQuery(trim($this->input->post('answer_hi',TRUE))),
		         	    'answer_en'         => cleanQuery(trim($this->input->post('answer_en',TRUE))),
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/faq/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['CategoryList'] = $this->FaqModel->GenerateDDList($this->__catTbl,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1,'is_delete'=>0));
		$this->data['DataList'] = $this->FaqModel->getSingleList($this->__table,array('id'=>$this->__id));
	
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
			
		/****Validation Rules start****/
		 $this->form_validation->set_rules('title_hi', 'Quesrion (Hindi)', 'trim|required|min_length[2]|max_length[255]');	
		 $this->form_validation->set_rules('title_en', 'Question (English)', 'trim|required|min_length[2]|max_length[255]');
                 $this->form_validation->set_rules('answer_hi', 'Answer (Hindi)', 'trim|required|min_length[2]');	
		 $this->form_validation->set_rules('answer_en', 'Answer (English)', 'trim|required|min_length[2]');
		 $this->form_validation->set_rules('category', 'Category', 'trim|required|is_natural_no_zero');
		 if($this->__allowStatus==1){
		 	$this->form_validation->set_rules('status', 'Status', 'trim|required|in_list[0,1]');	
		 	$this->__status = (int)cleanQuery($this->input->post('status'));	 
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Faq/edit/'.$this->__encId.'/');
         }else{
         	
         	 $this->__currentUrl = 'manage/Faq/edit/'.$this->__encId.'/';
		     ...
		     //end check file name 
				...
				$this->__queryStatus = $this->FaqModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/faq/edit',$this->data);
	}//end edit function
	...
	protected function _uploadFile($preUploadedFile=""){
		...
		if(isset($_FILES['attachment']['name'])==TRUE && trim($_FILES['attachment']['name'])!=""){
			...
			if(trim($preUploadedFile)!="" && trim($preUploadedFile)!=NULL){
				@unlink("./uploads/faq/".trim($preUploadedFile));
			}
    ...
    public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('order_number', 'Order Number', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->FaqModel->update_sort_order($update_id,$update_order,$this->__table);
	...
	public function recycle(){
		...
		$this->data['DataList'] = $this->FaqModel->getAllList($this->__table,$filter,$order);
		
	  	$this->front_view('admin/faq/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
			...
			$Query = $this->FaqModel->getSingleList($this->__table,$filter);
			
			if($this->FaqModel->deletedata($this->__table,$filter)==TRUE){
				
			   $Image = $Query->attachment ;
			   if(trim($Image)!=''){	
			    	if(is_file('./uploads/faq/'.$Image)){
