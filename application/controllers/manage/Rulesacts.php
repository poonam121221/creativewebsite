...
class Rulesacts extends Admin_Controller{
	...
	public function index(){  
		$this->data['CategoryList'] = $this->RulesactsModel->GenerateDDList($this->__catTable,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1));
        $this->front_view('admin/rules_acts/index',$this->data);	  	
	}//end index function
	...
	public function ajaxPaginationData(){
        ...
        //total rows count
        $totalRec = count($this->RulesactsModel->getRows($conditions,$filter));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url()."manage/Rulesacts/ajaxPaginationData";
        ...
        //load the view
        $this->load->view('admin/rules_acts/ajaxpaginationrules', $this->data, false);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Rulesacts/add/');
         }else{
				
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         		$result =  $this->RulesactsModel->getSingleList($this->__table);
         		
         		$this->__currentUrl = current_url();
         		$attachmentName = $this->_uploadFile();
         	    $DATAINPUT = array(
         	    	'cat_id'       => (int)cleanQuery(trim($this->input->post('cat_id',TRUE))),
				...
				$this->__queryStatus = $this->RulesactsModel->insertdata($this->__table,$DATAINPUT);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/rules_acts/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['DataList'] = $this->RulesactsModel->getSingleList($this->__table,array('id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Rulesacts/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->RulesactsModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/rules_acts/edit',$this->data);
	}//end edit function
	...
	public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('cat_id', 'Category', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->RulesactsModel->update_cat_sort_order($update_id,$update_order,$this->__table,'id','cat_id',$update_cat_id);
	...
	public function recycle(){  
		$this->data['CategoryList'] = $this->RulesactsModel->GenerateDDList($this->__catTable,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1));
        $this->front_view('admin/rules_acts/recycle',$this->data);	  	
	}//end index function
	...
	public function ajaxPaginationRecycle(){
        ...
        //load the view
        $this->load->view('admin/rules_acts/ajaxpaginationrulesrecycle', $this->data, false);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Sorry!'));
			 redirect('manage/Rulesacts/recycle');
		}
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	if($this->RulesactsModel->updatedata($this->__table,$data,$filter)==TRUE){
			...
			$Query = $this->RulesactsModel->getSingleList($this->__table,$filter);
			
			if($this->RulesactsModel->deletedata($this->__table,$filter)==TRUE){	
