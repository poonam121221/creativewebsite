...
class Rti extends Admin_Controller{
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('manage/RtiModel');
	...
	public function index(){  
        $this->front_view('admin/rti/index',$this->data);
	...
	public function ajaxPaginationData(){
        ...
        //total rows count
        $totalRec = count($this->RtiModel->getRows($conditions,$filter));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url("manage/Rti/ajaxPaginationData");
        ...
        //get posts data
        $this->data['DataList'] = $this->RtiModel->getRows($conditions,$filter,$orderBy);
        $this->data['PageNo'] = $offset;
        
        //load the view
        $this->load->view('admin/rti/ajaxpaginationrti', $this->data, false);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Rti/add/');
         }else{
				
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         		$result =  $this->RtiModel->getSingleList($this->__table);
				...
				$this->__queryStatus = $this->RtiModel->insertdata($this->__table,$DATAINPUT);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/rti/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['DataList'] = $this->RtiModel->getSingleList($this->__table,array('id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Rti/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->RtiModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/rti/edit',$this->data);
	}//end edit function
	...
	public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('order_number', 'Order Number', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->RtiModel->update_sort_order($update_id,$update_order,$this->__table);
	...
	public function recycle(){  
        ...
        //get the posts data
        $filter = array('ra.is_delete'=>1);
        $order  = array('ra.order_preference'=>'asc');
        
        $this->data['DataList'] = $this->RtiModel->getAllList($this->__table,$filter,$order);        
        //load the view
        $this->front_view('admin/rti/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	if($this->RtiModel->updatedata($this->__table,$data,$filter)==TRUE){
			...
			$Query = $this->RtiModel->getSingleList($this->__table,$filter);
			
			if($this->RtiModel->deletedata($this->__table,$filter)==TRUE){
