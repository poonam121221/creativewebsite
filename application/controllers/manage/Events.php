...
class Events extends Admin_Controller{
	...
	public function __construct(){
		...
		parent::__construct();
		$this->load->model('manage/EventsModel');
	...
	public function index(){
	    $this->front_view('admin/events/index',$this->data);
	}//end index function
	...
	public function ajaxPaginationData(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){
	        ...
	        $orderBy = array('ev.order_preference'=>'asc');
	        $conditions['table'] = $this->__table;
	        
	        //total rows count
	        $totalRec = count($this->EventsModel->ajax_search_by_title($conditions,$filter));
	        
	        //pagination configuration
	        $config['target']      = '#ajaxdata';
	        $config['base_url']    = base_url("manage/Events/ajaxPaginationData");
	        ...
	        //load the view
	        $this->load->view('admin/events/ajaxpaginationevents', $this->data, false);
	...
	public function add(){
		...
		$this->data['CategoryList'] = $this->EventsModel->GenerateDDList($this->__catTbl,'cat_id','cat_title_en','--SELECT CATEGORY--',array('cat_status'=>1,'is_delete'=>0));
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
           ...
           $DATAINPUT = array(	
			...
			$this->__queryStatus = $this->EventsModel->insertdata($this->__table,$DATAINPUT);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/events/add',$this->data);
	...
	public function edit(){
		...
		$this->data['DataList'] = $this->EventsModel->getSingleList($this->__table,array('id'=>$this->__id));
			...
			if ($this->input->server('REQUEST_METHOD') == 'POST'){
			 ...
			 if ($this->form_validation->run() == FALSE){
	             ...
	             redirect('manage/Events/edit/'.$this->__encId.'/');
	         }else{
			 	...
			 	$this->__queryStatus = $this->EventsModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/events/edit',$this->data);
	}//end edit function
	...
	public function delete(){
		...
		if($this->EventsModel->updatedata($this->__table,$DATAINPUT,$filter)==TRUE){
		...
		if(isset($_FILES['attachment']['name'])==TRUE && trim($_FILES['attachment']['name'])!=""){
			...
			if($this->upload->do_upload('attachment')){
				...
				if(trim($preUploadedFile)!="" && trim($preUploadedFile)!=NULL){
					@unlink("./uploads/events/".trim($preUploadedFile));
				}			
			}else{
	...
	public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('order_number', 'Order Number', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->EventsModel->update_sort_order($update_id,$update_order,$this->__table);
	...
	public function recycle(){
		...
		$this->data['DataList'] = $this->EventsModel->getAllList($this->__table,$filter,$order);
	    $this->front_view('admin/events/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Something went wrong, try again!'));
			 redirect('manage/Events/recycle');
		}
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	if($this->EventsModel->updatedata($this->__table,$data,$filter)==TRUE){
			...
			$Query = $this->EventsModel->getSingleList($this->__table,$filter);
			
			if($this->EventsModel->deletedata($this->__table,$filter)==TRUE){	
			
			   $Image = $Query->attachment ;
			   if(trim($Image)!=''){	
			    	if(is_file('./uploads/events/'.$Image)){
