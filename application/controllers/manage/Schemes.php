...
class Schemes extends Admin_Controller{
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('manage/SchemesModel');
	...
	public function index(){  
        $this->front_view('admin/schemes/index',$this->data);
	...
	public function ajaxPaginationData(){
        ...
        //total rows count
        $totalRec = count($this->SchemesModel->getRows($conditions,$filter));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url()."manage/Schemes/ajaxPaginationData";
        ...
        //get posts data
        $this->data['DataList'] = $this->SchemesModel->getRows($conditions,$filter,$orderBy);
        $this->data['PageNo'] = $offset;
        
        //load the view
        $this->load->view('admin/schemes/ajaxpaginationscheme', $this->data, false);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Schemes/add/');
         }else{
				
         		$UserLogId =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
         		$result =  $this->SchemesModel->getSingleList($this->__table);
				...
				$this->__queryStatus = $this->SchemesModel->insertdata($this->__table,$DATAINPUT);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/schemes/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['DataList'] = $this->SchemesModel->getSingleList($this->__table,array('id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Schemes/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->SchemesModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/schemes/edit',$this->data);
	}//end edit function
	...
	public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('order_number', 'Order Number', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->SchemesModel->update_sort_order($update_id,$update_order,$this->__table);
	...
	public function recycle(){  
        ...
        $this->data['DataList'] = $this->SchemesModel->getAllList($this->__table,$filter,$order);        
        //load the view
        $this->front_view('admin/schemes/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Sorry!'));
			 redirect('manage/Schemes/recycle');
		}
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	if($this->SchemesModel->updatedata($this->__table,$data,$filter)==TRUE){
			...
			$Query = $this->SchemesModel->getSingleList($this->__table,$filter);
			
			if($this->SchemesModel->deletedata($this->__table,$filter)==TRUE){
