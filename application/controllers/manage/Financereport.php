...
class Financereport extends Admin_Controller{
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('manage/FinancereportModel');
	...
	public function index(){  
        //load the view
        $this->front_view('admin/financereport/index',$this->data);
	...
	public function ajaxPaginationData(){
        ...
        //total rows count
        $totalRec = count($this->FinancereportModel->getRows($conditions,$filter));
        
        //pagination configuration
        $config['target']      = '#ajaxdata';
        $config['base_url']    = base_url("manage/Financereport/ajaxPaginationData");
        ...
        //get posts data
        $this->data['DataList'] = $this->FinancereportModel->getRows($conditions,$filter,$orderBy);
        $this->data['PageNo'] = $offset;
        
        //load the view
        $this->load->view('admin/financereport/ajaxpaginationfinancereport', $this->data, false);
	...
	public function add(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST'){ 
		 ...
		 if ($this->form_validation->run() == FALSE){
                ...
                redirect('manage/Financereport/add/');
         }else{
				...
				$this->__queryStatus = $this->FinancereportModel->insertdata($this->__table,$DATAINPUT);
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/financereport/add',$this->data);
	}//end add function
	...
	public function edit(){
		...
		$this->data['DataList'] = $this->FinancereportModel->getSingleList($this->__table,array('id'=>$this->__id));
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST'){
		 ...
		 if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('AppMessage',array('class'=>'warning','message'=>validation_errors()));
                redirect('manage/Financereport/edit/'.$this->__encId.'/');
         }else{
         		$UserLogId  =  encrypt_decrypt("decrypt",$this->session->userdata['AUTH_USER']['SERIALNO']);
				...
				$this->__queryStatus = $this->FinancereportModel->updatedata($this->__table,$DATAINPUT,array('id'=>$this->__id));
		...
		$this->data['optstatus'] = $this->__allowStatus;
	  	$this->front_view('admin/financereport/edit',$this->data);
	}//end edit function
	...
	public function updatesrtorder(){
	  ...
	  $this->form_validation->set_rules('order_number', 'Order Number', 'trim|required|is_natural_no_zero');
	  	 ...
	  	 if ($this->form_validation->run() == FALSE){
			...
			$this->__queryStatus = $this->FinancereportModel->update_sort_order($update_id,$update_order,$this->__table);
	...
	public function recycle(){  
        ...
        $this->data['DataList'] = $this->FinancereportModel->getAllList($this->__table,$filter,$order);  
        //load the view
        $this->front_view('admin/financereport/recycle',$this->data);
	...
	public function recycle_delete(){
		...
		if(($this->__id == NULL || $this->__id ==FALSE ||  $this->__id == "")){
			$this->session->set_flashdata('AppMessage',array('class'=>'danger','message'=>'Sorry!'));
			 redirect('manage/Financereport/recycle');
		}
		...
		$filter = array('id'=>$this->__id);
		 ...
		 if($del_no==0){
		 	if($this->FinancereportModel->updatedata($this->__table,$data,$filter)==TRUE){
			...
			$Query = $this->FinancereportModel->getSingleList($this->__table,$filter);
			
			if($this->FinancereportModel->deletedata($this->__table,$filter)==TRUE){	
